    ...
    public $action;
      ...
      $this->action = $func;
					...
					<script type="text/javascript">
						jQuery(document).ready(function() {
							...
							foreach ($this->actions as $action_name => $action) { ?>
								jQuery('<option>').val('<?php echo esc_js($action_name); ?>').text('<?php echo esc_js($action["menu_text"]); ?>').appendTo("select[name='action']");
								jQuery('<option>').val('<?php echo esc_js($action_name); ?>').text('<?php echo esc_js($action["menu_text"]); ?>').appendTo("select[name='action2']");
							<?php } ?>
						});
					</script>
				<?php
        ...
        $sendback = esc_url(home_url("/")) . $sendback;
				...
				?><script> window.location = '<?php echo esc_js($sendback);?>'; </script><?php
		...
		function custom_bulk_admin_notices() {
          ...
          if (!$message) return;
					...
					if( is_array($message) ){
						$message = sprintf( _n( $message['single'], $message['plural'], $post_ids_count, 'narratium' ), $post_ids_count );
					}
